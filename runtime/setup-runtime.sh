...
if [[ ! -e "$SUDOERS_FILE" ]] || ! /usr/bin/sudo -n -l /usr/bin/pmset -a disablesleep 0 >/dev/null 2>&1 || ! /usr/bin/sudo -n -l /usr/bin/pmset -a disablesleep 1 >/dev/null 2>&1; then
...
# Let System Settings resolve the app's name and icon instead of the CLI name.
...
/usr/bin/plutil -insert ProgramArguments.0 -string "$INSTALL_BIN" "$LAUNCH_AGENT"
/usr/bin/plutil -insert ProgramArguments.1 -string _recover "$LAUNCH_AGENT"
