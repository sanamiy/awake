...
残量がバッテリー下限を超えているか、電源接続中なら、スリープ防止が有効になり開始音が鳴ります。バッテリー駆動中に残量が下限以下なら、音を鳴らさず画面ロックのみ行い、蓋を閉じるなど通常の条件でスリープします。
...
| 設定 | 内容 | 初期値 |
| --- | --- | --- |
| 開始ショートカット | Awake Modeを開始するキー | ⌃⌘W |
| スリープ防止時間 | スライダーで15分・30分・1時間・2時間・4時間・8時間・24時間・無制限から選択 | 120分 |
| バッテリー下限 | バッテリー駆動中に停止する残量。スライダーで5〜95%を5%刻みで選択 | 30% |

スリープ防止時間を「無制限」にしても、ロック解除・バッテリー駆動中の残量下限への到達・アプリ終了でAwake Modeは終了します。
...
削除する場合は、Awakeの設定画面を開き、画面上部の「Awake」メニューから「アンインストール…」を選んでください。設定の削除後、「Finderで表示して終了」を押し、FinderでAwakeをゴミ箱へ移してください。時間やショートカットの設定値は、再インストールに備えて残ります。
