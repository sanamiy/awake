...
final class AppDelegateTests: AppModelTestCase {
    func testMenuStartsAwakeModeThroughTheModelAndDisablesWhileBusy() async throws {
        ...
        let uninstall = try XCTUnwrap(menu.items.first { $0.action == #selector(AppDelegate.requestUninstall) })
        XCTAssertTrue(uninstall.target === delegate)
        XCTAssertEqual(uninstall.keyEquivalent, "")
        XCTAssertGreaterThan(menu.index(of: uninstall), menu.index(of: quit))
        XCTAssertEqual(uninstall.image?.size, NSSize(width: 16, height: 16))
        XCTAssertTrue(try XCTUnwrap(uninstall.image).representations.isEmpty)
